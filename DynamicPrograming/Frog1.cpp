...
// rec
// T.C =  O(~ 2^n  approx
int minCostRec(int n, vi arr){
    if(n == 0) return n;

    int cost = 1e9;
    cost = min(cost , minCostRec(n-1 , arr) + abs(arr[n-1] - arr[n]));
    if(n > 1)
        cost = min(cost , minCostRec(n-2 , arr) + abs(arr[n-2] - arr[n]));
...
// T.C = N
int dp[N];
int minCost(int n, vi arr){
    if(n == 0) return n;
    if(dp[n] != -1) return dp[n];
    int cost = 1e9;
    cost = min(cost , minCost(n-1 , arr) + abs(arr[n-1] - arr[n]));
    if(n > 1)
        cost = min(cost , minCost(n-2 , arr) + abs(arr[n-2] - arr[n]));
